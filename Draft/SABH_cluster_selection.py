...
from tqdm import trange  # 加在文件顶部
...
def sabha_cluster_pipeline(
    ...
    alpha: float,                          # 目标 FDR
    gamma: float,                          # pi0 修正强度
    batch_size: int                        # KMeans 小批量大小
) -> np.ndarray:
    """
    reject_pixel_mask: shape (B, H, W), bool, 是否拒绝该像素所在的簇
    """
    ...
    for c in trange(C, desc="Step 2: Avg prob per cluster"):
        m = (cluster_id_map == c)
        if m.sum() == 0:
            continue
    ...
    # Step 6: SABHA 选择 + 映射回像素级别
    reject_clusters = sabha(cluster_pvals, pi0_adj, alpha)
    reject_pixel_mask = reject_clusters[cluster_id_map]

    return reject_pixel_mask
